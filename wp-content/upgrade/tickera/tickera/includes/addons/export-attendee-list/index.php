				...
				foreach ( $ticket_instances as $ticket_instance ) {
					...
					if ( $event_name == $event_id ) {
						...
						if ( $order_is_paid ) {
							...
							if ( isset( $_POST[ 'col_ticket_type' ] ) ) {
								$rows .= '<td>' . apply_filters( 'tc_checkout_owner_info_ticket_title', $ticket_type->details->post_title, isset( $instance->details->ticket_type_id ) ? $instance->details->ticket_type_id : $ticket_type->details->ID  ) . '</td>';
							}
							if ( isset( $_POST[ 'col_buyer_name' ] ) ) {
