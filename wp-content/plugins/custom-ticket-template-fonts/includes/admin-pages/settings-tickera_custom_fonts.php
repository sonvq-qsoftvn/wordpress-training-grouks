...
if ( isset( $_POST[ 'add_new_custom_font' ] ) ) {
	if ( check_admin_referer( 'save_custom_font' ) ) {
		if ( current_user_can( 'manage_options' ) || current_user_can( 'add_custom_font_cap' ) ) {
			...
			$message = __( 'Custom Font data has been successfully saved.', 'tc' );
		} else {
...
if ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'delete' ) {
	if ( !isset( $_POST[ '_wpnonce' ] ) ) {
		...
		if ( current_user_can( 'manage_options' ) || current_user_can( 'delete_custom_font_cap' ) ) {
			...
			$message	 = __( 'Custom Font has been successfully deleted.', 'tc' );
		} else {
			$message = __( 'You do not have required permissions for this action.', 'tc' );
		}
	}
...
<div class="wrap tc_wrap">
    <h2><?php if ( isset( $_GET[ 'action' ] ) && $_GET[ 'action' ] == 'edit' ) { ?><a href="edit.php?post_type=tc_events&page=<?php echo $_GET[ 'page' ] . '&tab=' . $_GET[ 'tab' ]; ?>" class="add-new-h2"><?php _e( 'Add New', 'tc' ); ?></a><?php } ?></h2>
    ...
    <form action="" method="post" enctype = "multipart/form-data">
		...
		<?php submit_button( (isset( $_REQUEST[ 'action' ] ) && $_REQUEST[ 'action' ] == 'edit' ? __( 'Update', 'tc' ) : __( 'Add New', 'tc' ) ), 'primary', 'add_new_custom_font', true ); ?>
        ...
        <div class="alignright actions new-actions">
            <form method="get" action="?page=<?php echo esc_attr( $page ); ?>" class="search-form">
                <p class="search-box">
                    ...
                    <label class="screen-reader-text"><?php _e( 'Search Custom Fonts', 'tc' ); ?>:</label>
                    <input type="text" value="<?php echo esc_attr( $custom_fonts_search ); ?>" name="s">
                    <input type="submit" class="button" value="<?php _e( 'Search Custom Fonts', 'tc' ); ?>">
                </p>
            </form>
        </div><!--/alignright-->
    ...
    <table cellspacing="0" class="widefat shadow-table">
        ...
        <tbody>
				...
				<tr id='user-<?php echo $custom_font_object->ID; ?>' <?php echo $style; ?>>
					...
					foreach ( $columns as $key => $col ) {
						if ( $key == 'edit' ) {
							?>
							<td>                    
								<a class="custom_fonts_edit_link" href="<?php echo admin_url( 'edit.php?post_type=tc_events&page=' . $tc->name . '_settings&tab=' . $_GET[ 'tab' ] . '&action=' . $key . '&ID=' . $custom_font_object->ID ); ?>"><?php _e( 'Edit', 'tc' ); ?></a>
							</td>
						<?php } elseif ( $key == 'delete' ) {
							?>
							<td>
								<a class="custom_fonts_edit_link tc_delete_link" href="<?php echo wp_nonce_url( 'edit.php?post_type=tc_events&page=' . $tc->name . '_settings&tab=' . $_GET[ 'tab' ] . '&action=' . $key . '&ID=' . $custom_font_object->ID, 'delete_' . $custom_font_object->ID ); ?>"><?php _e( 'Delete', 'tc' ); ?></a>
							</td>
							<?php
						} else {
							?>
			...
			if ( count( $wp_custom_fonts_search->get_results() ) == 0 ) {
				?>
				<tr>
					<td colspan="6"><div class="zero-records"><?php _e( 'No Custom Fonts found.', 'tc' ) ?></div></td>
				</tr>
				<?php
			}
			?>
