...
class TC_Upcoming_Events_Page_Widget extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget widget_recent_entries tc_upcoming_events_page_widget', 'description' => __( 'Shows upcoming events in event page', 'tc' ) );
		parent::__construct( 'TC_Upcoming_Events_Page_Widget', __( 'Upcoming Events Page', 'tc' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		global $tc;
            ...
            $tc_events_args_all = $tc_events_args;
            $tc_events_args_all['posts_per_page'] = -1;

			$tc_events = get_posts( $tc_events_args );
            $tc_events_all = get_posts( $tc_events_args_all );
			...
			if ( !empty( $tc_events ) ) {
				...
				<ul class='tc_upcoming_events_ul'>
					...
					foreach ( $tc_events as $tc_event ) {
						$event_content = '<li id="tc_upcoming_event_' . $tc_event->ID . '">
                                                    <span class="single-event-container">
                                                        <span class="event-date">' .
                                                            do_shortcode( '[tc_event_date_short id="' . $tc_event->ID . '"]' )                                
                                                        . '</span>
                                                        <span class="event-content">
                                                            <a href="' . get_post_permalink( $tc_event->ID ) . '">' . get_the_title( $tc_event->ID ) . '</a>
                                                            <span class="event-short-desc">' . wp_trim_words( $tc_event->post_content, 7, '' ) . '</span>
                                                            <span class="tc_event_data_widget">' . 
                                                                do_shortcode( '[tc_event_date_blocks id="' . $tc_event->ID . '"]' ) 
                                                                . '<a class="see-detail-event-button" href="' . get_post_permalink( $tc_event->ID ) . '">En savoir +</a>
                                                            </span>                                                            
                                                        </span>
                                                    </span>
						</li>';
						echo apply_filters( 'tc_upcoming_events_widget_page_event_content', $event_content, $tc_event->ID );
					}
					?>
				</ul><!--tc_cart_ul-->
				<?php
                ...
                if ((count($tc_events_all) > 5) && !isset($_GET['show'])) {
                    echo '<a class="more-event-button" href="' . get_page_link(51) . '?show=all' . '">SEE MORE EVENTS</a>';
                }
			} else {
...
add_action( 'widgets_init', create_function( '', 'return register_widget("TC_Upcoming_Events_Page_Widget");' ) );
?>