...
// meta caps (don't assign these to roles)
			...
			register_post_status( 'order_fraud', array(
				'label'			 => __( 'Fraud', 'mp' ),
				'label_count'	 => _n_noop( 'Fraud <span class="count">(%s)</span>', 'Fraud <span class="count">(%s)</span>', 'mp' ),
				'post_type'		 => 'tc_orders',
				'public'		 => true
			) );
	...
	function tc_plugin_updater( $plugin_slug = '', $plugin_file = '' ) {
		...
		if ( !defined( 'TC_NU' ) ) {//updates are allowed
			...
			if ( file_exists( $updater_file ) ) {
				...
				$tc_plugin_update_checker = PucFactory::buildUpdateChecker( 'https://tickera.com/update/?action=get_metadata&slug=' . $plugin_slug . '&license_key=' . $license_key, $plugin_file, $plugin_slug, 1 );
			}
		}
	}
